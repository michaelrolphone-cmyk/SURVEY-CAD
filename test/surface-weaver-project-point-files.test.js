...
test('SURFACE.html can load project point files for an active project', async () => {
  ...
  assert.match(html, /const\s+columnIndex\s*=\s*\{\s*point:\s*0,\s*north:\s*1,\s*east:\s*2,\s*elev:\s*3,\s*descStart:\s*4\s*\};/, 'Surface Weaver should default CSV columns to P,N,E,Z when no header mapping is available');
  assert.match(html, /const\s+northIdx\s*=\s*findIndex\(\"n\",\s*\"north\",\s*\"northing\",\s*\"y\"\);[\s\S]*const\s+eastIdx\s*=\s*findIndex\(\"e\",\s*\"east\",\s*\"easting\",\s*\"x\"\);/, 'Surface Weaver should map northing from y-style headers and easting from x-style headers');
  assert.match(html, /const\s+N\s*=\s*toNum\(row\[columnIndex\.north\]\);[\s\S]*const\s+E\s*=\s*toNum\(row\[columnIndex\.east\]\);/, 'Surface Weaver should read northing and easting from the resolved header-mapped columns');
  assert.match(html, /const\s+x\s*=\s*\(p\.E\s*-\s*offE\);[\s\S]*const\s+z\s*=\s*-\(p\.N\s*-\s*offN\);/, 'Surface Weaver should invert plotted northing on the Z axis so point geometry aligns with north-up map orientation');
  assert.match(html, /const\s+tex\s*=\s*new\s+THREE\.CanvasTexture\(out\);[\s\S]*tex\.flipY\s*=\s*false;/, 'Surface Weaver should disable vertical texture flipping so map imagery stays correctly oriented after northing-axis inversion');
  ...
  assert.match(html, /function\s+updateOrbitDistanceLimits\(box3\)\s*\{[\s\S]*const\s+diagonal\s*=\s*Math\.max\(size\.length\(\),\s*1\);[\s\S]*const\s+boundingRadius\s*=\s*diagonal\s*\*\s*0\.5;[\s\S]*orbit\.minDistance\s*=\s*Math\.max\(0\.5,\s*boundingRadius\s*\*\s*1\.05,\s*maxDim\s*\*\s*0\.18\);[\s\S]*orbit\.maxDistance\s*=\s*Math\.max\(diagonal\s*\*\s*600,\s*orbit\.minDistance\s*\*\s*30,\s*12000\);/, 'Surface Weaver should keep orbit zoom from entering the surface while preserving a broad zoom-out range');
