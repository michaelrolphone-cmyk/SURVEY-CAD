...
test('SURFACE.html can load project point files for an active project', async () => {
  ...
  assert.match(html, /function\s+updateOrbitDistanceLimits\(box3\)\s*\{[\s\S]*orbit\.minDistance\s*=\s*Math\.max\(0\.25,\s*maxDim\s*\*\s*0\.02\);[\s\S]*orbit\.maxDistance\s*=\s*Math\.max\(maxDim\s*\*\s*250,\s*orbit\.minDistance\s*\*\s*12,\s*3000\);/, 'Surface Weaver should clamp orbit zoom distance to prevent zooming from locking pan/rotate interactions');
