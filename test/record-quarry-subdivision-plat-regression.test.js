...
test('RecordQuarry builds subdivision plat doc-id index and caps nearby subdivision results', async () => {
  ...
  assert.match(html, /function\s+findPreferredSubdivisionEntryIndex\s*\([\s\S]*const\s+parcelCandidates\s*=\s*getSubdivisionNameCandidates\(parcelAttrs\);/, 'RecordQuarry should derive parcel subdivision name candidates when choosing which nearby subdivision plat to show first.');
  assert.match(html, /if\s*\(bestScore\s*<\s*0\.67\)\s*return\s*-1;/, 'RecordQuarry should only prioritize a nearby subdivision plat when parcel-to-subdivision name matching is high confidence.');
  assert.match(html, /function\s+prioritizeParcelSubdivisionEntry\s*\([\s\S]*source\[preferredIndex\],[\s\S]*\.\.\.source\.slice\(0,\s*preferredIndex\)/, 'RecordQuarry should move the parcel-matching subdivision entry to the front of the subdivision plat list while preserving the remaining order.');
  ...
  assert.match(html, /state\.nearbySubdivisions\s*=\s*prioritizeParcelSubdivisionEntry\(state\.nearbySubdivisions,\s*parcel\);/, 'RecordQuarry should prioritize the subdivision plat that matches the parcel subdivision metadata before rendering cards.');
  assert.match(html, /const\s+prioritizedSubdivisionCards\s*=\s*prioritizeParcelSubdivisionEntry\(subdivisionCards,\s*parcel\);/, 'RecordQuarry should apply parcel subdivision prioritization to the rendered subdivision card list.');
