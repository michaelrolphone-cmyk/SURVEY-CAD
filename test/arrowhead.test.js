...
test('ArrowHead mobile AR app reads LineSmith payload and projects using bearing/elevation camera angles', async () => {
  ...
  assert.match(html, /const\s+orientationPermissionGranted\s*=\s*await\s+requestOrientationPermissionIfNeeded\(\);[\s\S]*const\s+motionPermissionGranted\s*=\s*await\s+requestMotionPermissionIfNeeded\(\);/, 'ArrowHead should explicitly request both orientation and motion permissions during startup');
  assert.match(html, /await\s+startCamera\(\);[\s\S]*const\s+orientationPermissionGranted\s*=\s*await\s+requestOrientationPermissionIfNeeded\(\);/, 'ArrowHead should start camera capture before sensor permission checks so denied sensors do not block video');
  ...
  assert.match(html, /const\s+geolocationPermissionGranted\s*=\s*await\s+requestInitialGeolocationPermissionIfNeeded\(\);/, 'ArrowHead startup should request geolocation permission before attaching ongoing GPS watches');
