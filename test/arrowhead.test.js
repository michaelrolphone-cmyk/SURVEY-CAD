...
test('ArrowHead mobile AR app reads LineSmith payload and projects using bearing/elevation camera angles', async () => {
  ...
  assert.match(html, /if \(!state\.useGyroscopeHeading && shouldApplyHeadingUpdate && Number\.isFinite\(pose\.headingRad\)\) \{[\s\S]*state\.headingRad = pose\.headingRad;[\s\S]*\}/, 'ArrowHead should default to magnetometer heading updates unless gyroscope mode is enabled');
  ...
  assert.match(html, /const\s+shouldApplyHeadingUpdate\s*=\s*shouldApplyOrientationEvent\([\s\S]*\);/, 'ArrowHead should gate heading updates per-event without dropping every orientation callback');
  assert.match(html, /if \(!state\.useGyroscopeHeading && shouldApplyHeadingUpdate && Number\.isFinite\(pose\.headingRad\)\) \{[\s\S]*state\.headingRad = pose\.headingRad;[\s\S]*\}/, 'ArrowHead should only gate heading updates behind absolute-heading lock logic');
  assert.match(html, /if \(!isAbsoluteOnlyEvent \|\| !state\.hasAbsoluteHeadingLock\) \{[\s\S]*state\.pitchRad = pose\.pitchRad;[\s\S]*state\.rollRad = pose\.rollRad;[\s\S]*\}/, 'ArrowHead should continue applying non-absolute pitch/roll updates while absolute heading lock is active');
