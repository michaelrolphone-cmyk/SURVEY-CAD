...
test('EvidenceDesk uses project point file API endpoints for point-file list and deletion', async () => {
  ...
  assert.match(html, /async function\s+deleteServerUploadResource\(projectContext,\s*sourceFolderKey,\s*entry\)/, 'EvidenceDesk should centralize server-upload deletion through project-files endpoint.');
  assert.match(html, /const\s+sendDeleteRequest\s*=\s*async\s*\(folderKey,\s*fileName\)\s*=>\s*\{[\s\S]*fetch\(deleteUrl\.toString\(\),\s*\{\s*method:\s*'DELETE'\s*\}\)/, 'EvidenceDesk should delete server-upload records through DELETE project-files endpoint.');
  assert.match(html, /firstAttempt\.response\.status\s*===\s*404[\s\S]*shouldRetryWithFallback[\s\S]*sendDeleteRequest\(fallbackLocation\.folderKey,\s*fallbackLocation\.storedName\)/, 'EvidenceDesk should retry server-upload deletes using fallback URL-derived location details when metadata is stale.');
  assert.match(html, /entry\?\.reference\?\.type\s*===\s*'server-upload'[\s\S]*await\s+deleteServerUploadResource\(projectContext,\s*folder\?\.key,\s*entry\)/, 'EvidenceDesk project deletion should delete server uploads before removing them from project state.');
  ...
  assert.match(html, /const\s+pointFileAssociationMarkup\s*=\s*folder\.key\s*===\s*'point-files'\s*&&\s*pointFileAssociations\.length[\s\S]*↳ Drawing:/, 'EvidenceDesk point-file rows should show an indented list of associated drawings.');
  assert.match(html, /const\s+drawingAssociationMarkup\s*=\s*folder\.key\s*===\s*'drawings'\s*&&\s*drawingAssociationName[\s\S]*↳ Point file:/, 'EvidenceDesk drawing rows should show the associated point file in an indented insert.');
...
test('EvidenceDesk file rows keep controls aligned and always visible', async () => {
  ...
  assert.match(html, /resource\.innerHTML\s*=\s*`<span class="file-meta">\$\{leadingIcon\}\$\{thumbnailSlotMarkup\}<span class="file-name"><span class="file-name-primary"><span class="file-name-configured" title="\$\{actualFileName\}">\$\{configuredFileName\}<\/span>\$\{rosPillMarkup\}\$\{pointPillMarkup\}\$\{equipmentAuditPillMarkup\}<\/span>\$\{pointFileAssociationMarkup\}\$\{drawingAssociationMarkup\}<\/span><\/span>`;/, 'EvidenceDesk should render file names with nested metadata and association inserts.');
