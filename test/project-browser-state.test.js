...
test('Project Browser can open CP&F rows as PDF links in a new tab', async () => {
  ...
  assert.match(projectBrowserHtml, /async\s+function\s+openFieldBookPrintPreview\s*\(/, 'Project Browser should define an async bulk field-book print-preview builder');
  ...
  assert.match(projectBrowserHtml, /\.filter\(\(entry\) => isResourceStarredForFieldBook\(entry\)\)/, 'Print preview should include only starred field-book records');
  ...
  assert.match(projectBrowserHtml, /printAllButton\.addEventListener\('click',\s*\(\)\s*=>\s*openFieldBookPrintPreview\(folder\.key, folder\.index\)\)/, 'Print starred action should open a combined field-book print preview');
  ...
  assert.match(projectBrowserHtml, /printStarredBundleButton\.textContent\s*=\s*'Print all starred'/, 'Evidence Desk header should render a master Print all starred action next to folder creation controls.');
  assert.match(projectBrowserHtml, /printStarredBundleButton\.addEventListener\('click',\s*\(\)\s*=>\s*openMasterFieldBookPrintPreview\(projectContext\.projectFile\)\)/, 'Master print action should open a combined print bundle for all starred field-book folders.');
  assert.match(projectBrowserHtml, /function\s+getMasterFieldBookBundleResources\s*\([\s\S]*targetFolderKeys\s*=\s*\['plats',\s*'ros',\s*'cpfs'\]/, 'Project Browser should gather master print resources from plats, ROS, and CP&F folders.');
  assert.match(projectBrowserHtml, /function\s+syncMasterPrintStarredButtonState\s*\(button,\s*projectFile\s*=\s*\{\}\)\s*\{[\s\S]*getMasterFieldBookBundleResources\(projectFile\)/, 'Master print button state should disable when no starred printable bundle entries are available.');
  assert.match(projectBrowserHtml, /async\s+function\s+openMasterFieldBookPrintPreview\s*\([\s\S]*No starred Plats, ROS, or CP&F files were found/, 'Master print preview should provide a clear error when no starred bundle resources exist.');
  assert.match(projectBrowserHtml, /Print all starred Plats, ROS, and CP&amp;Fs/, 'Master print preview toolbar should label the combined field-book print bundle.');
  assert.match(projectBrowserHtml, /if \(folderKey === 'plats'\) \{[\s\S]*getPdfPreviewUrl\(\{ key: 'plats' \}, entry\) \|\| getImagePreviewUrl\(entry\) \|\| getExternalResourceUrl\(entry\)/, 'Plat print URLs should prefer generated thumbnail/image previews before falling back to raw external sources.');
