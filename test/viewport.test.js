...
test('VIEWPORT.HTML exposes an FLD editor with local override save/reset and downloads', async () => {
  ...
  assert.match(html, /function\s+getSymbolMapFileForRule\(rule\)\s*\{[\s\S]*fieldToFinishRuleState\.symbolSvgOverrides\.get\(symbolName\)/, 'FLD editor should resolve symbol SVG mappings from symbol-name local overrides first');
  ...
  assert.match(html, /onPick:\s*\(value\) => \{[\s\S]*setSymbolMapFileForRule\(rule, value\);[\s\S]*applyFieldToFinishConfig\(fldEditorState\.activeConfig, fldEditorState\.source === "local" \? "localStorage" : defaultFldConfigPath\);/, 'FLD editor SVG picker should persist symbol-to-SVG overrides without mutating FLD row mappings');
...
test('VIEWPORT.HTML formats bearings in DMS symbols and groups shared-bearing segments with aggregate labels', async () => {
  ...
  assert.match(html, /function\s+lineIsEligibleForBearingLabels\(line,\s*startPoint,\s*endPoint\)\s*\{[\s\S]*line\.fieldToFinishBaseCode[\s\S]*splitCodeTokens\(startPoint\?\.code\)[\s\S]*startTokens\.includes\("BEAR"\)\s*\|\|\s*endTokens\.includes\("BEAR"\);/, 'bearing labels should allow listed linework codes and BEAR point-code overrides');
  assert.match(html, /if\s*\(lineIsEligibleForBearingLabels\(ln,\s*a,\s*b\)\)\s*\{[\s\S]*lineLabelCandidates\.push\(\{\s*a,\s*b,\s*sa,\s*sb\s*\}\);[\s\S]*\}/, 'draw loop should only queue bearing labels for eligible coded lines');
