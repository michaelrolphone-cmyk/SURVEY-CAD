...
test('VIEWPORT.HTML parses generic field-to-finish commands for sequential BEG/END/CLO workflows', async () => {
  ...
  assert.match(html, /const\s+pointImplicitSequenceCodes\s*=\s*\[\.\.\.tokenSet\]\.filter\(\(token\)\s*=>\s*\([\s\S]*lineworkCodes\.has\(token\)\s*\|\|\s*explicitSequentialBaseCodes\.has\(token\)[\s\S]*&&\s*!curveLineworkCodes\.has\(token\)[\s\S]*\)\);/, 'LineSmith should skip implicit straight segments for linework codes marked as PC/PT curve geometry');
  ...
  assert.match(html, /function\s+buildFieldToFinishCurveLineCommands\(\)\s*\{[\s\S]*activeCurve = \{ startPointId: point\.id, middlePointId: null \};[\s\S]*lineCommands\.push\(\{[\s\S]*a: activeCurve\.startPointId,[\s\S]*b: point\.id,[\s\S]*middlePointId: activeCurve\.middlePointId,[\s\S]*baseCode: "CURVE"[\s\S]*\}\);/, 'LineSmith should build three-point PC-to-PT curve commands and retain the middle tangent point id');
  ...
  assert.match(html, /const\s+existingLine\s*=\s*findDirectConnectionLine\(cmd\.a, cmd\.b\);[\s\S]*if \(cmd\.type === "curve" && existingLine\.generatedBy === "field-to-finish" && existingLine\.fieldToFinishType !== "curve"\)[\s\S]*existingLine\.fieldToFinishType = "curve";[\s\S]*existingLine\.fieldToFinishCurveMiddlePointId = Number\.isFinite\(Number\(cmd\.middlePointId\)\) \? Number\(cmd\.middlePointId\) : null;/, 'field-to-finish sync should promote existing auto-lines into curve lines so PC/PT curves render on first draw state');
  ...
  assert.match(html, /function\s+traceLinePath\(context, line, startScreen, endScreen, projectPoint = worldToScreen\)\s*\{[\s\S]*quadraticCurveTo\(middleScreen\.x - tangentX, middleScreen\.y - tangentY, middleScreen\.x, middleScreen\.y\);[\s\S]*quadraticCurveTo\(middleScreen\.x \+ tangentX, middleScreen\.y \+ tangentY, endScreen\.x, endScreen\.y\);/, 'LineSmith should render field-to-finish curve lines as smooth two-segment quadratic paths through the PC/PT middle point');
