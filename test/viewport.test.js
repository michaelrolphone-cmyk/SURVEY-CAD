...
test('VIEWPORT.HTML clusters nearby points with hover details, double-click zoom, and spread labels', async () => {
  ...
  assert.match(html, /function\s+showPointHoverTooltip\(point,\s*screenX\s*=\s*mouse\.x,\s*screenY\s*=\s*mouse\.y\)\s*\{[\s\S]*const\s+pointDelta\s*=\s*basePoint && basePoint\.id !== point\.id \? lineMeasurement\(basePoint, point\) : null;[\s\S]*Distance:[\s\S]*Bearing:/, 'single-point hover tooltip should include inverse distance and bearing rows when hovering away from a single selected point');
