...
test('VIEWPORT.HTML parses generic field-to-finish commands for sequential BEG/END/CLO workflows', async () => {
  ...
  assert.match(html, /if \(!directives\.has\(first\) && directives\.has\(second\)\) \{[\s\S]*baseCode = first;[\s\S]*action = second;[\s\S]*\} else if \(directives\.has\(first\) && !directives\.has\(second\)\) \{[\s\S]*baseCode = second;[\s\S]*action = first;/, 'field-to-finish parser should support both "CODE BEG" and "BEG CODE" sequential directive ordering');
  ...
  assert.match(html, /const\s+pointImplicitSequenceCodes\s*=\s*\[\.\.\.tokenSet\][\s\S]*resolveLineworkSequenceKey\(token, explicitSequentialBaseCodes\)[\s\S]*!curveLineworkCodes\.has\(tokenBaseCode\)/, 'LineSmith should build implicit sequence keys from resolved linework tokens so numbered variants remain unique and curve-only codes are excluded');
  assert.match(html, /function\s+pointSupportsLineworkSequenceToken\(tokenSet, sequenceKey\)\s*\{[\s\S]*if \(!parsed\.numericSuffix\) \{[\s\S]*tokenParsed\.numericSuffix && tokenParsed\.baseCode === parsed\.baseCode[\s\S]*\} else \{[\s\S]*return false;[\s\S]*\}[\s\S]*lineworkCompanionCodes\.get\(parsed\.baseCode\)/, 'LineSmith should keep numbered sequences isolated while allowing unnumbered base codes to continue across numbered variants and still honor FLD companion tokens');
