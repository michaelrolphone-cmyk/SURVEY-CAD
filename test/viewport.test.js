...
test('VIEWPORT.HTML right-click marquee zoom uses the latest cursor position and includes window padding', async () => {
  ...
  assert.match(html, /const\s+marqueeRect\s*=\s*rectNorm\(mouse\.dragObj\.x0, mouse\.dragObj\.y0, mouse\.dragObj\.x1, mouse\.dragObj\.y1\);[\s\S]*pendingMarqueeZoomRect\s*=\s*marqueeRect;[\s\S]*applyMarqueeSelection\(mouse\.dragObj\);/, 'marquee mouseup should cache the just-drawn window so a follow-up right-click can still zoom to that window');
  assert.match(html, /const\s+pendingWindowRect\s*=\s*consumePendingMarqueeZoomRect\(\);[\s\S]*if \(pendingWindowRect\) \{[\s\S]*zoomToScreenRect\(pendingWindowRect\);/, 'right-click should prioritize cached marquee-window zoom before falling back to cancel/clear behavior');
});
