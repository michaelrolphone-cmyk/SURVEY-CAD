...
test('VIEWPORT.HTML parses JPN references and auto-connects matching point numbers during import', async () => {
  ...
  assert.match(html, /function\s+buildExpectedJpnLineCommands\(\)\s*\{[\s\S]*extractJpnTargetPointNumbers\(sourcePoint\.code\)[\s\S]*commands\.push\(\{ a: sourcePoint\.id, b: targetPointId, type: \"jpn\" \}\);/, 'LineSmith should derive expected JPN linework commands between source points and referenced targets');
...
test('VIEWPORT.HTML parses generic field-to-finish commands for sequential BEG/END/CLO workflows', async () => {
  ...
  assert.match(html, /Maintain sequence continuity for normal linework codes even when a point[\s\S]*if \(pointLineworkCodes\.includes\(baseCode\)\) continue;[\s\S]*activeSequences\.delete\(baseCode\);/, 'points with JPN links should still preserve sequential line continuity for active linework codes');
