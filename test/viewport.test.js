...
test('VIEWPORT.HTML parses generic field-to-finish commands for sequential BEG/END/CLO workflows', async () => {
  ...
  assert.match(html, /const\s+ruleLayerId\s*=\s*resolveFieldToFinishLayerIdForCode\(code\);[\s\S]*addPoint\(\{ num, x:\s*easting, y:\s*northing, z:\s*0, code, notes, movable:false, layerId: ruleLayerId \|\| selectedLayerId \}\);/, 'CSV import should map canonical northing/easting into drawing y/x while assigning FLD-configured layers');
  ...
  assert.match(html, /function\s+buildFieldToFinishCurveLineCommands\(\)\s*\{[\s\S]*pointHasCurveStartToken\(point\.code\)[\s\S]*lineCommands\.push\(\{ a: activeCurve\.lastPointId, b: point\.id, baseCode: "CURVE" \}\);[\s\S]*pointHasCurveEndToken\(point\.code\)/, 'LineSmith should build curve linework segments from PC-to-PT runs, including all intermediate approximation points');
