...
test('VIEWPORT.HTML parses generic field-to-finish commands for sequential BEG/END/CLO workflows', async () => {
  ...
  assert.match(html, /const\s+fieldToFinishRuleState\s*=\s*\{[\s\S]*lineworkCodes:\s*new\s+Set\(\),[\s\S]*symbolCodes:\s*new\s+Set\(\),[\s\S]*lineworkCompanionCodes:\s*new\s+Map\(\),[\s\S]*companionToLineworkCodes:\s*new\s+Map\(\)/, 'LineSmith should maintain FLD-derived linework/symbol code sets and companion-code lookups for field-to-finish behavior');
