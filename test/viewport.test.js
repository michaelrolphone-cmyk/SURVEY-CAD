...
test('VIEWPORT.HTML clusters nearby points with hover details, double-click zoom, and spread labels', async () => {
  ...
  assert.match(html, /const\s+CLUSTER_SINGLE_CLICK_SELECTION_DELAY_MS\s*=\s*275\s*;/, 'cluster point selection should use a short delay so double-click zoom can win before opening the point drawer');
  ...
  assert.match(html, /canvas\.addEventListener\("dblclick", \(e\) => \{[\s\S]*clearPendingClusterPrimaryAction\(\);[\s\S]*const\s+cluster\s*=\s*pointDisplayVisibility\.clustering\s*\?\s*getPointClusterAtScreen\(mouse\.x,\s*mouse\.y\)\s*:\s*null;/, 'cluster double-click handling should cancel pending single-click selection before zooming');
  assert.match(html, /if \(tool === "select" && pointDisplayVisibility\.clustering\) \{[\s\S]*const\s+cluster\s*=\s*getPointClusterAtScreen\(mouse\.x,\s*mouse\.y\);[\s\S]*cluster && cluster\.members\?\.length > 1[\s\S]*scheduleClusterPrimaryAction\(\{ additive, forcedPointId: closestMember\.point\.id \}\);[\s\S]*return;[\s\S]*\}/, 'single-clicks inside multi-point clusters should be delayed and resolve to the nearest cluster member');
...
test('VIEWPORT.HTML adds layer model, toolbar controls, and layer manager modal for drawing rules', async () => {
  ...
  assert.match(html, /const\s+type\s*=\s*fieldToFinishRuleState\.lineworkCodes\.has\(baseCode\)[\s\S]*"linework"[\s\S]*fieldToFinishRuleState\.symbolCodes\.has\(baseCode\)[\s\S]*"symbol"/, 'buildQuickPointGroups should classify each group as linework, symbol, or other based on FLD rule state');
  assert.match(html, /function\s+getGroupLineworkThumbnailDataUrl\(baseCode\s*=\s*""\)\s*\{[\s\S]*renderLineworkThumbnailDataUrl[\s\S]*lineworkCodes:\s*new\s+Set\(\[baseCode\]\)/, 'point-group dropdown should generate field-to-finish linework thumbnails for codes with configured linework entity types');
  assert.match(html, /function\s+getSymbolFallbackDotDataUrl\(\)\s*\{[\s\S]*circle[\s\S]*fill/, 'point-group dropdown should generate a filled circle dot thumbnail as fallback for symbol-type codes without an SVG mapping');
  assert.match(html, /group\.type\s*===\s*"linework"[\s\S]*getGroupLineworkThumbnailDataUrl\(group\.baseCode\)[\s\S]*group\.type\s*===\s*"symbol"[\s\S]*group\.symbolUrl\s*\|\|\s*getSymbolFallbackDotDataUrl\(\)/, 'point-group dropdown should display field-to-finish thumbnails for linework groups and SVG or dot fallback for symbol groups');
...
test('VIEWPORT.HTML renders configured survey symbol SVG markers for point codes before falling back to x markers', async () => {
  ...
  assert.match(html, /function\s+applyFieldToFinishConfig\(config, sourceLabel = defaultFldConfigPath\)\s*\{[\s\S]*fieldToFinishRuleState\.loaded = true;[\s\S]*reapplyFieldToFinishLayerMappings\(\);/, 'field-to-finish config apply should re-map existing point/line layers to updated FLD layer assignments');
  assert.match(html, /function\s+reapplyFieldToFinishLayerMappings\(\)\s*\{[\s\S]*for \(const point of points\.values\(\)\) \{[\s\S]*resolveFieldToFinishLayerIdForCode\(point\.code \|\| ""\);[\s\S]*for \(const line of lines\.values\(\)\) \{[\s\S]*if \(line\.generatedBy !== "field-to-finish"\) continue;[\s\S]*resolveAutoLineLayerId\(line\.a, line\.b, \{ type, baseCode, sourcePointId \}\);/, 'field-to-finish layer remap helper should update both point symbol layers and generated linework layers');
  assert.match(html, /line\.fieldToFinishSourcePointId\s*=\s*Number\.isFinite\(Number\(sourcePointId\)\)\s*\?\s*Number\(sourcePointId\)\s*:\s*null;/, 'auto-generated field-to-finish lines should persist source point ids so remapping can keep JPN layer ownership stable');
