...
test('VIEWPORT.HTML exposes an FLD editor with local override save/reset and downloads', async () => {
  ...
  assert.match(html, /function\s+normalizeLineworkRuleDefaults\(rule\)\s*\{[\s\S]*rule\.raw\.symbol = "SPT10";[\s\S]*rule\.raw\.symbol_name_2 = "";/, 'FLD editor should enforce default linework symbols and remove symbol SVG mappings for line entities');
