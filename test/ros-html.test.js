...
test('RecordQuarry.html loads CP&F PDF links when a corner marker is selected', async () => {
  ...
  assert.match(html, /record\.distanceMeters\s*==\s*null\s*\|\|\s*record\.distanceMeters\s*<=\s*\(maxMeters\s*\+\s*0\.5\)/, 'corner CP&F lookup should filter to records near the selected corner instead of section-wide results');
