...
test('RecordQuarry.html can export unique boundary points directly to PointForge', async () => {
  ...
  assert.match(html, /const\s+notesByCoordinate\s*=\s*await\s*buildCpfNotesByCoordinate\(plssPoints\);[\s\S]*includePlssWithoutNotes:\s*false[\s\S]*buildPointMarkerCsvRowsPNEZD\(utilityMarkers,\s*uniquePart\.nextPoint,\s*'UTILITY'\)[\s\S]*const\s+pointForgeCsv\s*=\s*`\$\{uniquePart\.csv\}\$\{utilityRows\.csv\}`[\s\S]*localStorage\.setItem\(POINTFORGE_ROS_IMPORT_STORAGE_KEY,\s*JSON\.stringify\(\{[\s\S]*csv:\s*pointForgeCsv/, 'ROS should prefetch CP&F notes, append Idaho Power utility points, and persist PointForge payload CSV');
