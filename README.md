...
`/api/localstorage-sync` traffic is change-driven in the launcher: after an initial forced POST when the launcher first loads, the app skips follow-up POST requests when the local snapshot is unchanged. Stale snapshot application updates key/value pairs without forcing iframe reloads or wiping tool-specific localStorage entries that are missing from the server snapshot. This prevents AR and viewport sessions from being reset by background sync churn while still propagating updated geometry/state data.
