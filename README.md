...
- Field-to-Finish layer color assignment: when FLD layer creation happens on-the-fly, utility-like names use standard colors (`Water` → blue, `Gas` → yellow, `Sewer` → green, `Power/Electric` → red), while other new FLD-created layers are assigned a random high-visibility color.
...
- Selection-aware active layer toolbar behavior: when you select a point/line, the quick layer selector temporarily shows that entity's layer; when the selection clears, the selector returns to the previously active drawing layer. If a mixed selection spans different layers, the selector label shows `Multiple Layers`.
- API/CLI surface impact: no new endpoints or commands were introduced for this layer-toolbar selection behavior; continue using existing API endpoints (for example `GET /api/fld-config`) and existing CLI commands (`npm run cli -- --help`, `npm run ros:cli -- --help`, `npm run icons:generate`).
...
- Right-click marquee zoom fallback: after finishing a marquee drag, the next right-click still zooms to that last-drawn window instead of clearing selection, matching the expected LineSmith/AutoCAD-style zoom-window workflow.
- Cluster double-click zoom now clears active point/line selection before framing the cluster extent, so using cluster zoom does not unintentionally leave a nearby point selected.
- API/CLI surface impact: no new endpoints or commands were introduced for this cluster zoom selection-clearing behavior; continue using existing endpoints (`GET /api/fld-config`) and existing CLI commands (`npm run cli -- --help`, `npm run ros:cli -- --help`, `npm run icons:generate`).
...
- While hovering a cluster, LineSmith now identifies the member point closest to the cursor, highlights that point on the canvas, and marks the matching tooltip row as active so you can confirm which point will be selected next.
...
- LineSmith exposes **Open ArrowHead AR** from both the Project Drawing panel and the top quick-toolbar (`quickOpenArrowHead`) so ArrowHead can be launched as a second live viewport.
- ArrowHead reads this payload and overlays LineSmith points/linework on a live camera feed.
- When ArrowHead opens without an active LineSmith handoff payload, it now falls back to the active project's last-opened LineSmith drawing (`surveyfoundryLastLineSmithDrawing:<projectId>`) and reconstructs AR import geometry from saved drawing history.
...
- ArrowHead now locks onto absolute heading streams (compass/`deviceorientationabsolute`) once detected, but automatically falls back to relative orientation events when absolute updates stall so heading does not freeze after a single absolute sample.
- ArrowHead now ignores null/non-numeric orientation sensor fields (for example `webkitCompassHeading: null`) so live heading updates continue from valid `alpha` samples instead of freezing after the first reading.
- ArrowHead now wraps geolocation/orientation/motion event callbacks in guarded handlers and makes websocket presence sends non-throwing, so a single runtime event exception no longer stalls subsequent sensor updates.
...
- ArrowHead now raises a blocking `window.alert(...)` when camera startup throws, including the specific thrown error name/message (and fallback attempt context) so iOS 13.6 users without console access can report exact failure details.
- Quick launch command for camera troubleshooting: `open "http://localhost:3000/ArrowHead.html"` then tap **Start AR Sensors** to trigger camera startup and any alert diagnostics.
- ArrowHead HUD now includes a live debug panel that prints GPS coordinates/altitude, heading-pitch-roll, projected vs total point counts, projection drop reasons, and distance/bearing to the nearest point so iOS field testing can explain why geometry is not rendering.
- Debug command: `open "http://localhost:3000/ArrowHead.html"` and tap **Start AR Sensors** to populate the debug panel while moving the device.
...
```bash
# Launch from LineSmith handoff payload
...
Upstream HTTP failures from third-party services (for example, geocoding provider 403s) are returned as `502 Bad Gateway` from this API so callers can distinguish dependency outages from client-side request validation errors. Geocoding now tries Nominatim first and then automatically falls back to ArcGIS World Geocode when Nominatim fails. `/api/lookup` will still return a successful payload when geocoding fails but the Ada County address layer returns a match (including a fallback query that relaxes directional/suffix filters). If both data sources fail to locate the address, `/api/lookup` returns a clear validation error instead of bubbling an upstream HTTP error. `/api/lookup` now also attempts Idaho Power utility lookup by calling `https://api.idahopower.com/serviceEstimator/api/NearPoint/Residential/PrimaryPoints/<x>/<y>/` for the address origin plus surrounding offsets (500 ft N/E/S/W and 700 ft NE/NW/SE/SW), normalizes returned primary-point geometry (including payloads wrapped under `object.primaryPoints`/`object.transformers`), applies Idaho Power service code labels (`1` → `PM`, `2` → `UP`, `3` → `OH`, transformer features → `TRANSF`), and includes a deduplicated `utilities` array in the response for RecordQuarry map/export workflows. Utility endpoint points include a `code` field for preview mapping, and PointForge export keeps service labels directly (`PM`, `OH`, `UP`, `TRANSF`) without injecting `JPN` join-point notation. `/api/utilities` degrades gracefully by returning `{ "utilities": [] }` when Idaho Power endpoints are unavailable.
...
`/api/localstorage-sync` traffic is change-driven in the launcher: after an initial forced POST when the launcher first loads, the app skips follow-up POST requests when the local snapshot is unchanged. During those unchanged intervals it now performs a lightweight `GET /api/localstorage-sync` check and applies newer server snapshots (by version) so users still converge on the same shared dataset even if they have no local edits. Stale snapshot application updates key/value pairs without forcing iframe reloads or wiping tool-specific localStorage entries that are missing from the server snapshot. This prevents AR and viewport sessions from being reset by background sync churn while still propagating updated geometry/state data.
...
- `loadUtilitiesByAddress(address, outSR?)` → `/api/utilities` (Idaho Power utility lookup calls NearPoint Residential PrimaryPoints (`/NearPoint/Residential/PrimaryPoints/<x>/<y>/`) at origin + eight nearby offsets, labels returned service points with PM/OH/UP utility codes (and transformers as `TRANSF`), deduplicates merged responses, and normalizes results for RecordQuarry preview markers and PointForge export points)
...
LineSmith now defaults **point notes** visibility to off on load, no longer shows the workflow toast while the active tool is **Select / Move**, and removes the **Center (0,0)** toolbar shortcuts (Zoom Extents and selection-centered actions remain available).
No API endpoints or CLI commands changed in this UI update; existing `npm start`, `npm test`, and `npm run cli -- ...` workflows are unchanged.
ArrowHead now includes iOS Safari 13-compatible CSS viewport pinning fallbacks (`top/right/bottom/left` alongside `inset`) so the HUD/action buttons render instead of a blank screen on older iPhones.
No API endpoints or CLI commands changed in this compatibility fix; existing `npm start`, `npm test`, and `npm run cli -- ...` workflows remain the same.
No API endpoints or CLI commands changed in this sensor-permission fallback fix; existing commands remain unchanged (`npm start`, `npm test`, `npm run cli -- --help`, `npm run ros:cli -- --help`).
RecordQuarry/PointForge ROS boundary export now guards against swapped parcel/subdivision feature payloads so parcel vertices keep `COR` labels and subdivision vertices keep `SUB` labels even when upstream feature slots are accidentally inverted during lookup composition.
No API endpoints or CLI commands changed in this boundary-label regression fix; existing commands remain unchanged (`npm start`, `npm test`, `npm run cli -- --help`, `npm run ros:cli -- --help`).
RecordQuarry now refetches subdivision export geometry by containing feature `OBJECTID` directly in EPSG:2243 before CSV/PointForge export, preventing subdivision vertices from being emitted at fallback WGS84 scale when upstream projection responses degrade.
No API endpoints or CLI commands changed in this subdivision-scale export fix; existing commands remain unchanged (`npm start`, `npm test`, `npm run cli -- --help`, `npm run ros:cli -- --help`).
RecordQuarry ROS boundary export now uses area-aware parcel/subdivision swap protection when payload attributes are ambiguous, so parcel corners stay labeled `COR` and subdivision corners stay labeled `SUB` in PointForge/CSV output.
No API endpoints or CLI commands changed in this parcel/subdivision code-label regression fix; existing commands remain unchanged (`npm start`, `npm test`, `npm run cli -- --help`, `npm run ros:cli -- --help`).

RecordQuarry/PointForge export now also applies an area-ratio fallback when parcel/subdivision attributes are missing, preventing swapped feature slots from labeling parcel corners as `SUB` in PointForge/CSV output.
No API endpoints or CLI commands changed in this parcel/subdivision disambiguation fix; existing commands remain unchanged (`npm start`, `npm test`, `npm run cli -- --help`, `npm run ros:cli -- --help`).
Idaho Power NearPoint utility normalization now enforces service point types from service type IDs (`1=PM`, `2=UP`, `3=OH`), labels transformer utilities as `TRANSF`, and keeps exported PointForge utility codes free of `JPN` join-point notation.
No API endpoints or CLI commands changed in this utility point-labeling fix; existing commands remain unchanged (`npm start`, `npm test`, `npm run cli -- --help`, `npm run ros:cli -- --help`).
...
When the map layer is enabled, LineSmith now preserves the current canvas viewport and only toggles the basemap visibility so turning map tiles on does not force a zoom-extents recenter.
LineSmith map tiles now overzoom well past native tile availability (extended Leaflet render zoom range up to 40) by reusing/scaling the last available zoom level when necessary, so users can keep zooming while drawn points stay aligned to the basemap instead of map zoom freezing at native tile limits.
No API endpoints or CLI commands changed in this map overzoom fallback update; existing commands remain unchanged (`npm start`, `npm test`, `npm run cli -- --help`, `npm run ros:cli -- --help`).
...
PointForge export now keeps Idaho Power utility point descriptions as direct utility codes (`PM`, `OH`, `UP`, `TRANSF`) without `JPN` junction references or `BEG`/`END` suffixes.
...
LineSmith controls drawer now includes an inner collapsible **Tool drawer sections** group so most controls can be hidden quickly, while **Map opacity**, the multi-point distance/bearing inspector, and **Point inspector** remain pinned in an always-visible inspector section (with line inspector shown above point details for quicker distance checks).
...
- **LineSmith** (`VIEWPORT.HTML`): Turns points into boundaries, alignments, and structure; now supports mobile-first canvas gestures (pinch-zoom, drag-pan, long-press select/box-select), plus long-press add-point placement when the Add Point tool is active, with a slide-out tools drawer. Desktop blank-canvas left-drag marquee selection remains enabled (Shift adds to selection). On desktop, the inspector/controls drawer now has a left-edge collapse tab (hanging over the canvas boundary) and a right-screen cutout expand tab for quick hide/show toggling. The drawer panel allows horizontal overflow so the collapse tab remains visible instead of being clipped. Collaboration now broadcasts live cursor updates for touch/pointer movement and continuously syncs drag edits (point/line moves) so remote peers see in-progress and committed geometry movement in real time. Collaboration state sync now intentionally excludes viewport pan/zoom (including Zoom to Extents) so each connected user keeps their own local view while shared geometry and selections still sync.
  ...
  - Field-to-finish parsing now uses an extensible command-parser pipeline: sequential line commands like `COR BEG`, `COR END`, and `COR CLO` are interpreted during import/editor updates to build `COR` linework in point order (including close-to-begin behavior), and this parser registry is designed for adding future point-code directives without rewriting import flow.
