...
- `loadUtilitiesByAddress(address, outSR?)` → `/api/utilities` (Idaho Power utility lookup calls NearPoint Residential PrimaryPoints (`/NearPoint/Residential/PrimaryPoints/<x>/<y>/`), labels returned service points with PM/OH/UP utility codes using service-type rules, and normalizes results for RecordQuarry preview markers and PointForge export points, including normalized utility `code` labels used for map preview (PointForge export rewrites those into PM/JPN connection codes))
...
Idaho Power NearPoint utility normalization now enforces three point types from service type IDs (`1=PM`, `2=UP`, `3=OH`) and keeps `JPN` notation on the PM connection point only during PointForge export.
...
PointForge export now also rewrites Idaho Power utility point descriptions so only the structure connection point (`PM`) carries linked junction point numbers (`JPN<n>`), while downstream power utility points remain labeled by service type (`OH`/`UP`) without `JPN` or `BEG`/`END` suffixes.
