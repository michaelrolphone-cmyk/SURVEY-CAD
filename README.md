...
CP&F PDF scraping now runs as its own paged checkpoint flow, so the worker continues downloading every CP&F PDF even when parcel/feature-map harvesting checkpoints are already complete.
...
- `GET /api/projects/:projectId/point-files/:pointFileId`
- `GET /api/projects/:projectId/drawings/:drawingId`
- `GET /api/project-files/pdf-thumbnail?source=<pdf-url>` – triggers deduplicated thumbnail generation and returns: `202` while work is in progress, `200 image/png` when the cached 1024px-wide page-1 thumbnail is ready, or `404` with `{ status: "failed" }` when the source PDF no longer exists (`/api/project-files/download` only), or `502` with `{ status: "failed" }` for renderer/remote-fetch failures (cooldown before automatic retry). `source` may be a relative API URL (`/api/project-files/download...`, `/api/ros-pdf...`) or an absolute `http(s)` URL targeting either API path.
...
When `pointFileLink` is set, drawing saves also update `PUT/PATCH /api/projects/:projectId/point-files/:pointFileId` semantics automatically by appending a point-file version from the drawing's latest points. Linked point-file coordinate serialization/hydration uses surveying order `Point,Northing,Easting` (northing first), while LineSmith drawing state remains internal `x=easting`, `y=northing`. You can switch which point file a drawing uses by sending a new `pointFileLink` in `PATCH /api/projects/:projectId/drawings/:drawingId` without resending `drawingState`. PointForge→LineSmith launch payloads now include `pointFileLink`, so opening LineSmith from PointForge automatically carries the exported project point-file association.
...
- Stackhero MinIO keys are also supported directly: `STACKHERO_MINIO_HOST`, `STACKHERO_MINIO_ROOT_ACCESS_KEY`, `STACKHERO_MINIO_ROOT_SECRET_KEY`, plus optional `STACKHERO_MINIO_ACCESS_KEY`, `STACKHERO_MINIO_SECRET_KEY`, `STACKHERO_MINIO_PORT`, and `STACKHERO_MINIO_USE_SSL` (host is normalized to `https://...` by default when no scheme is provided; `STACKHERO_MINIO_USE_SSL=false` switches to `http://`, and non-default ports are appended to the endpoint).
