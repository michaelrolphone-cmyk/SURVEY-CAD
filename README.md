...
- `loadUtilitiesByAddress(address, outSR?)` → `/api/utilities` (Idaho Power utility lookup calls EstimateDetail `Calculate` for service types `1`, `2`, and `3`, then normalizes results for RecordQuarry preview markers and PointForge export points, including `code` labels for service-line begin/end markers)
