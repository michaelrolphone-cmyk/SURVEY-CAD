...
Upstream HTTP failures from third-party services (for example, geocoding provider 403s) are returned as `502 Bad Gateway` from this API so callers can distinguish dependency outages from client-side request validation errors. Geocoding now tries Nominatim first and then automatically falls back to ArcGIS World Geocode when Nominatim fails. `/api/lookup` will still return a successful payload when geocoding fails but the Ada County address layer returns a match (including a fallback query that relaxes directional/suffix filters). If both data sources fail to locate the address, `/api/lookup` returns a clear validation error instead of bubbling an upstream HTTP error. `/api/lookup` now also attempts Idaho Power utility lookup by posting to `https://api.idahopower.com/serviceEstimator/api/EstimateDetail/Calculate` (the same API family used by the Idaho Power Service Estimator client) with `serviceEstimateServiceTypeId` values `1`, `2`, and `3`, normalizes returned transformer/location geometry across those service types, applies OH/UG service code labels (`1` → `OH PWR`, `2/3` → `UG PWR`) for estimate-derived utility points, derives service-line endpoint utility points for overhead/underground lines, and includes a `utilities` array in the response for RecordQuarry map/export workflows. Utility endpoint points include a `code` field for preview mapping, while PointForge export rewrites power utility descriptions into connected junction notation (`PM JPN<n> ...` on the structure connection point and `JPN<n>` on downstream OH/UG points) so exports no longer depend on `BEG`/`END` labels. `/api/utilities` degrades gracefully by returning `{ "utilities": [] }` when Idaho Power endpoints are unavailable.
...
- `loadUtilitiesByAddress(address, outSR?)` → `/api/utilities` (Idaho Power utility lookup calls EstimateDetail `Calculate` for service types `1`, `2`, and `3`, labels estimate-derived points with OH/UG utility codes using those service-type rules, and normalizes results for RecordQuarry preview markers and PointForge export points, including normalized utility `code` labels used for map preview (PointForge export rewrites those into PM/JPN connection codes))
...
PointForge export now also rewrites Idaho Power utility point descriptions so the first exported utility point is the structure connection point (`PM`) annotated with linked junction point numbers (`JPN<n>`), while subsequent OH/UG points export as `JPN<n>` codes without `BEG`/`END` suffixes.
