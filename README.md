...
- ArrowHead camera startup now accepts both modern `navigator.mediaDevices.getUserMedia` and legacy `navigator.getUserMedia`/`navigator.webkitGetUserMedia`/`navigator.mozGetUserMedia`, preventing false "Camera API unavailable" errors on older iOS Safari builds that still expose prefixed camera APIs.
