...
- ArrowHead now renders the on-target leader connector as a fading tail from the screen center toward the target point, tapering to zero opacity at the point for less visual obstruction.
- API/CLI surface impact: no new endpoints or commands were introduced for this ArrowHead on-target tail rendering update; continue using existing API routes (for example `GET /api/fld-config`) and CLI commands (`npm run cli -- --help`, `npm run ros:cli -- --help`, `npm run icons:generate`).
